...
resource "aci_bridge_domain" "bridge_domain" {
  ...
  name               = each.key
  tenant_dn          = each.value.tenant_name.id  #aci_tenant.tenant.id
  ...
  relation_fv_rs_ctx = each.value.vrf #[for x in vrf.value["VRFS"] : try(var.vrf[x].id)]  #aci_vrf.vrf[each.value.vrf].id
}
